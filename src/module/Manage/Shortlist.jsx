        ...
        <div key={applicant.id} className="app-icon">
          ...
          <button onClick={()=>{
            ...
            if(!applicant.Shortlist){
              axios.patch(`http://localhost:8080/applicants/${applicant.id}`,{Shortlist:true})
              
            }
            else{
              axios.patch(`http://localhost:8080/applicants/${applicant.id}`,{Shortlist:false})
            }
          }}
          className={applicant.Shortlist ? 'Shortlisted':'Shortlist'}
          >{applicant.Shortlist ? 'Shortlisted':'Shortlist'}</button>
          <button onClick={()=>{
            setShort((prev)=>!prev);
             if(confirm("Are you sure you want to reomve this employee")){
              axios.delete(`http://localhost:8080/applicants/${applicant.id}`)
